...
function hideVideoNext_Prev() {
    'use strict';
    viewNext_Prev.style.display = 'none';
}
...
(function() {
    'use strict';
    video[rangeVolume.name] = rangeVolume.value;
}());
...
function toggleVolume() { // Run On Click On Icon Volume For Mute Volume
    ...
    if (buttonVolume.classList.contains('fa-volume-off')) {
        ...
        if (rangeVolume.value === 0) {
            ...
            rangeVolume.onmousemove = function() {
                video.muted = false;
                video.volume = this.value;
            };
        }
...
qualityOption.onclick = function() {
    'use strict';
    clickActiveQuality();
    setQuality();
};
qualityOptionAuto.onclick = function() {
    'use strict';
    clickActiveAutoQuality();
};
// Run Functions (Player) Full Screen
